...
function parsescript($script, $level) {
	...
	if (sizeof($script)) {
		foreach ($script as $block) {
			showtabs($level);
			if ($block[0] == 'procDef') {
				$GLOBALS['out'][] = $GLOBALS['curout'];
				$GLOBALS['curout'] = '';
				$GLOBALS['curout'] .=  'define ' . parseblock($block[1], $block[2]) . "\n";
			} else if ($block[0] == 'whenGreenFlag') {
				$GLOBALS['out'][] = $GLOBALS['curout'];
				$GLOBALS['curout'] = '';
				$GLOBALS['curout'] .=  'when gf clicked' . "\n";
			} else if (in_array($block[0], $cblocks)) {
				$blocktype = $block[0];
				$args = $block;
				array_shift($args);
				$GLOBALS['curout'] .=  parseblock($dictionary[$blocktype], $args) . "\n";
				...
				if (!isset($dictionary[$blocktype])) {
					$GLOBALS['curout'] .=  parseblock($blocktype, $block) . "\n";
				} else {
					$GLOBALS['curout'] .=  parseblock($dictionary[$blocktype], $block) . "\n";
				}
			}
		}
	}
